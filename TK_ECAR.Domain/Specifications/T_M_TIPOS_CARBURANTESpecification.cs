...
namespace TK_ECAR.Domain.Specifications
{
    ...
    [Serializable]
    public partial class T_M_TIPOS_CARBURANTESpecification : ISpecification<T_M_TIPOS_CARBURANTE>
    {
    
        public Nullable<int> ID_CARBURANTE
        {
            get; 
            set;
        }
    		
    	public IEnumerable<Nullable<int>> ID_CARBURANTEIN
    	{
    		get;
    		set;
    	}
    
    	public Nullable<int> ID_CARBURANTEFrom
    	{
    		get;
    		set;
    	}
    	
    	public Nullable<int> ID_CARBURANTETo
    	{
    		get;
    		set;
    	}
    	...
    	/// Initializes a new instance of the <see cref="T_M_TIPOS_CARBURANTESpecification"/> class.
    	/// </summary>
    	public T_M_TIPOS_CARBURANTESpecification()
    	{
    	...
    	public T_M_TIPOS_CARBURANTESpecification(bool initializeNavigationProperties)
    	{
    	...
    	public Expression<Func<T_M_TIPOS_CARBURANTE, bool>> GetExpression()
    	{
    		Expression<Func<T_M_TIPOS_CARBURANTE, bool>> expression = x => true;
    
    		if(ID_CARBURANTE.HasValue)
    			expression = expression.And(x => x.ID_CARBURANTE == ID_CARBURANTE.Value);
    
    		if(ID_CARBURANTEIN != null && ID_CARBURANTEIN.Count() > 0)
    			expression = expression.And(x => ID_CARBURANTEIN.Contains(x.ID_CARBURANTE));
    	
    		if(ID_CARBURANTEFrom.HasValue)
    			expression = expression.And(x => x.ID_CARBURANTE >= ID_CARBURANTEFrom.Value);
    			
    		if(ID_CARBURANTETo.HasValue)
    			expression = expression.And(x => x.ID_CARBURANTE <= ID_CARBURANTETo.Value);
    	...
    	public bool IsSatisfiedBy(T_M_TIPOS_CARBURANTE entity)
    	{
    		// convert single entity to a IQueryable object, 
    		// in order to be able to use lambda expressions
    		IQueryable<T_M_TIPOS_CARBURANTE> entities = (new[] { entity }).AsQueryable();
