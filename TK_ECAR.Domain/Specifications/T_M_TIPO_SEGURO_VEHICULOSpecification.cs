...
namespace TK_ECAR.Domain.Specifications
{
    ...
    [Serializable]
    public partial class T_M_TIPO_SEGURO_VEHICULOSpecification : ISpecification<T_M_TIPO_SEGURO_VEHICULO>
    {
    
        public Nullable<int> ID_SEGURO_VEHICULO
        {
            get; 
            set;
        }
    		
    	public IEnumerable<Nullable<int>> ID_SEGURO_VEHICULOIN
    	{
    		get;
    		set;
    	}
    
    	public Nullable<int> ID_SEGURO_VEHICULOFrom
    	{
    		get;
    		set;
    	}
    	
    	public Nullable<int> ID_SEGURO_VEHICULOTo
    	{
    		get;
    		set;
    	}
    	...
    	/// Initializes a new instance of the <see cref="T_M_TIPO_SEGURO_VEHICULOSpecification"/> class.
    	/// </summary>
    	public T_M_TIPO_SEGURO_VEHICULOSpecification()
    	{
    	...
    	public T_M_TIPO_SEGURO_VEHICULOSpecification(bool initializeNavigationProperties)
    	{
    	...
    	public Expression<Func<T_M_TIPO_SEGURO_VEHICULO, bool>> GetExpression()
    	{
    		Expression<Func<T_M_TIPO_SEGURO_VEHICULO, bool>> expression = x => true;
    
    		if(ID_SEGURO_VEHICULO.HasValue)
    			expression = expression.And(x => x.ID_SEGURO_VEHICULO == ID_SEGURO_VEHICULO.Value);
    
    		if(ID_SEGURO_VEHICULOIN != null && ID_SEGURO_VEHICULOIN.Count() > 0)
    			expression = expression.And(x => ID_SEGURO_VEHICULOIN.Contains(x.ID_SEGURO_VEHICULO));
    	
    		if(ID_SEGURO_VEHICULOFrom.HasValue)
    			expression = expression.And(x => x.ID_SEGURO_VEHICULO >= ID_SEGURO_VEHICULOFrom.Value);
    			
    		if(ID_SEGURO_VEHICULOTo.HasValue)
    			expression = expression.And(x => x.ID_SEGURO_VEHICULO <= ID_SEGURO_VEHICULOTo.Value);
    	...
    	public bool IsSatisfiedBy(T_M_TIPO_SEGURO_VEHICULO entity)
    	{
    		// convert single entity to a IQueryable object, 
    		// in order to be able to use lambda expressions
    		IQueryable<T_M_TIPO_SEGURO_VEHICULO> entities = (new[] { entity }).AsQueryable();
